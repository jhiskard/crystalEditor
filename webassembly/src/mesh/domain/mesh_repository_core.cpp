...
namespace {
MeshManager* g_meshManagerInstance = nullptr;
}
...
MeshManager::~MeshManager() {
    if (g_meshManagerInstance == this) {
        g_meshManagerInstance = nullptr;
    }
}
...
void MeshManager::PrintMeshTree() {
    if (g_meshManagerInstance == nullptr || g_meshManagerInstance->GetMeshTree() == nullptr) {
        return;
    }

    g_meshManagerInstance->GetMeshTree()->TraverseTree([](const TreeNode* node, void*) {
...
void MeshManager::ShowMesh(int32_t id) {
    ...
    m_MeshTree->TraverseTreeMutable([&](TreeNode* traverseNode, void*) {
        Mesh* mesh = GetMeshByIdMutable(traverseNode->GetId());
        if (mesh == nullptr) {
            return;
        }
        if (traverseNode->GetIconState() == IconState::HIDDEN ||
            traverseNode->GetIconState() == IconState::PARTIAL) {
            ...
            traverseNode->SetIconState(IconState::VISIBLE);
        }
    }, node);

    SetParentIconState(node);
}
...
void MeshManager::SetParentIconState(TreeNode* node) {
    ...
    if (g_meshManagerInstance == nullptr) {
        return;
    }

    IconState iconState = g_meshManagerInstance->GetChildrenVisibility(node->GetParent());
...
void MeshManager::DeleteMesh(int32_t id) {
    ...
    m_MeshTree->TraverseTreeMutable([&](TreeNode* node, void*) {
...
void MeshManager::ApplySharedVolumeDisplaySettingsToAllMeshes() {
    ...
    m_MeshTree->TraverseTree([&](const TreeNode* node, void*) {
        Mesh* mesh = GetMeshByIdMutable(node->GetId());
