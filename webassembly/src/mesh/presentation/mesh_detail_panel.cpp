...
// ============================================================================
namespace {
mesh::domain::MeshRepository& legacyMeshRepository() {
...
void forEachVolumeMesh(const std::function<void(Mesh*)>& func) {
    auto& meshRepository = legacyMeshRepository();
    meshRepository.GetMeshTree()->TraverseTree([&](const TreeNode* node, void*) {
...
void MeshDetail::Render(int32_t meshId, bool* openWindow) {
    ...
    auto& meshRepository = legacyMeshRepository();
    const Mesh* mesh = meshRepository.GetMeshById(meshId);
    if (mesh == nullptr) {
        ImGui::End();
        return;
    }
    ...
    if (mesh->GetVolumeMeshActor()) {
        if (!meshRepository.HasSharedVolumeDisplaySettings()) {
            meshRepository.EnsureSharedVolumeDisplaySettingsFromMesh(*mesh);
        }
        syncSharedVolumeUiFromSettings(meshRepository.GetSharedVolumeDisplaySettings());
    }
    ...
    if (isClicked) {
        ...
        if (linkToTree) {
            if (m_UiVolumeMeshVisibility) {
                meshRepository.ShowMesh(meshId);
            } else {
                meshRepository.HideMesh(meshId);
            }
        } else if (m_VolumeUseSharedSettings) {
            mesh::domain::MeshRepository::VolumeDisplaySettings settings = meshRepository.GetSharedVolumeDisplaySettings();
            settings.volumeVisibility = m_UiVolumeMeshVisibility;
            meshRepository.SetSharedVolumeDisplaySettings(settings);
...
void MeshDetail::renderTableRowVolumeWindowLevel(int32_t meshId) {
    ...
    auto& meshRepository = legacyMeshRepository();
    Mesh* mesh = meshRepository.GetMeshByIdMutable(meshId);
    
    double rangeMin = 0.0;
    double rangeMax = 1.0;
    if (m_VolumeUseSharedSettings) {
        meshRepository.GetGlobalVolumeDataRange(rangeMin, rangeMax);
    } else if (mesh) {
        const double* range = mesh->GetVolumeDataRange();
        if (range) {
            rangeMin = range[0];
...
// ============================================================================
void MeshDetail::syncSharedVolumeUiFromSettings(const mesh::domain::MeshRepository::VolumeDisplaySettings& settings) {
